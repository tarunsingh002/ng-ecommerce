...
export class AuthService {
  ...
  signUp(email: string, password: string) {
    const apiUrl =
      "https://identitytoolkit.googleapis.com/v1/accounts:signUp?key=AIzaSyAZ-GvOyC9d0qcSyE_tF3AaD96azhh6O1I";
  ...
  signIn(email: string, password: string) {
    const apiUrl =
      "https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyAZ-GvOyC9d0qcSyE_tF3AaD96azhh6O1I";
  ...
  handleAuthentication(
    ...
    if (this.signUpMode) {
      ...
      this.userS.loadUsers().subscribe((users) => {
        ...
        if (u.orders) user.orders = u.orders;
        else user.orders = [];
        user.rtdid = u.rtdid;
        this.User.next(user);
        this.autoLogout(expiresIn * 1000);
